...
@pytest.mark.asyncio
async def test_two_dif_folder_adding(create_user, create_folder, session):
    # Create a new user 
    user_name    = "test_user"
    user_chat_id = 1
    ...
    # Create a second new folder
    second_new_folder_name = new_folder_name[::-1]
    second_new_folder_path = f"{cur_folder_path}{second_new_folder_name}/"

    second_new_folder_id = await create_folder(
        user_id         = user_id,
        par_folder_id   = root_folder.id,
        new_folder_name = second_new_folder_name,
        new_folder_path = second_new_folder_path
    )
    ...
    await set_user_folder(
        session   = session,
        chat_id   = user_chat_id,
        folder_id = second_new_folder_id
    )
...
#---
    ...
    root_folder, new_folder, second_new_folder = folders

    assert user is not None
    assert user.folders is not None
    assert user.folders[1].folder_name      == new_folder_name
    assert user.folders[2].folder_name      == second_new_folder_name
    
    assert root_folder.id == second_new_folder.parent_folder_id
    assert root_folder.child_folders[1] == second_new_folder

@pytest.mark.asyncio
async def test_two_same_folder_adding(create_user, create_folder, session):
    # Create a new user 
    user_name    = "test_user"
    user_chat_id = 1
    ...
    # Check for existing previous new folder
    folder_id = await check_folder_by_path_and_chat_id(
        session   = session,
        path      = new_folder_path,
        chat_id   = user_chat_id
