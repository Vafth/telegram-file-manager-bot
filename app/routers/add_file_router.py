...
from ..db.db_interaction.check import check_user_file_and_file_folder_link 
from ..db.db_interaction.create import create_new_file
...
add_file_router = Router()
add_file_router.message.filter(userIsAllowed(), isPrivate())

@add_file_router.message(or_f((F.animation), (F.audio), (F.photo), (F.video), (F.sticker), (F.document)))
...
@add_file_router.message(F.text, FileAddingProccedData.file_name)
